    ...
    <div className='grid grid-cols-12 gap-4 p-8 '>
      {practice.hooks.map((hook) => (
        <Link
          href={`/${hook.name}`}
          key={hook.name}
          className='m-2 col lg:col-span-4 md:col-span-6 col-span-12'
        >
          <h2>{hook.name}</h2>
          <p className='my-6 col btn-2th'> Learn more about </p>
          <h5>{hook.description}</h5>
          <div className='w-full overflow-auto p-2 bg-gray-500 text-white card-rounded'>
            <pre>{hook.usage}</pre>
